...
| 8.2 disabled thinking | Use a small, **restricted, model-profile renderer** for exactly two string messages (system/user), no tools, `add_generation_prompt=true`, `enable_thinking=false`. Renderings below have been checked against pinned upstream Jinja for all 252 fixtures. Safe crate `apply_chat_template` does not accept kwargs; common's C++ Jinja API is not exposed by the sys wrapper. Do not add a C++ shim or general Jinja engine in v1. Template status is `exact` only for identical hashes. A nonidentical template is accepted as `reviewed-equivalent` only by a manifest record keyed to the exact GGUF artifact hash, GGUF template hash, and native-profile template hash, with a stated restricted scope and reproducible evidence; unseen hash triples fail explicitly. Unknown GGUFs require explicit `--template-profile qwen3|qwen3.5|minicpm5`; arbitrary template inference is rejected. Record that override, and do not claim unknown artifacts are golden-verified. |
...
M1 stores small test-oracle renders with source attribution, hashes and regeneration instructions outside reference. M2 compares registered model template metadata against expected profile semantics and records its hash; absence/mismatch is an explicit template error until reviewed against the pinned native oracle. Qwen3-0.6B is the sole reviewed nonidentical case: artifact `9465e63a…031`, GGUF template `57f1fd00…d0361` (4,100 bytes), and native profile `a55ee1b1…74d8` (4,168 bytes). Parent/Astra approved `reviewed-equivalent`, not exact, only for two string system/user messages, no tools, `add_generation_prompt=true`, and `enable_thinking=false`. The preserved fixtures and Jinja 3.1.4 oracle reproduce equality for 144 authored + 108 perturbation rows, every 252 reference prompt hash, and four edge states; this is not a claim about tools, multimodal content, assistant reasoning, or arbitrary multi-turn conversations. An arbitrary local/Hub model's explicit profile is a deliberate override recorded as such, with no parity claim.
...
Serialize pull for the same repo/revision/file with a process-safe lock; hf-hub handles its transfer temp/cache mechanics, our verified receipt is written atomically only after hash/size verification. Interrupted transfer is not a verified model. Concurrent-process and tamper tests must establish download-once behavior. Validate portable `OWNER/NAME` and safe relative nested filename components before every cache filesystem operation; absolute paths, dot/traversal components, backslashes, Windows drive/network forms, and empty components are invalid. Cache-controlled writes/removals must remain inside canonical ownership boundaries, and a snapshot symlink may be followed only when its regular-file target remains in the owned hub. Mismatched cached data returns an integrity error without repair. `models pull --repair` may quarantine corrupt **owned bytes**, remove/rebuild owned snapshots, and redownload exactly that pinned artifact once; it must bypass but never modify/quarantine a corrupt external HF source. Offline repair may reuse a valid alternate and otherwise returns `OfflineMiss` without network. Relative snapshot symlinks are resolved to canonical regular files before hard-link import, dangling destinations are handled with `symlink_metadata`, and no verified receipt survives a failed repair. Local/external files are never modified or quarantined automatically. Remote custom specs require a 40-hex commit and expected SHA-256; local custom files may omit expected hash, but compute actual hash and label integrity `local-unverified`. A model identity fingerprint includes bytes, template profile, tokenizer metadata and native build/config, not only a friendly ID. Local model metadata uses `source=local`, canonical path as file, and `revision=sha256:<actual hash>`; remote revision always denotes the GGUF repo commit, never the native BF16 commit (stored separately).
